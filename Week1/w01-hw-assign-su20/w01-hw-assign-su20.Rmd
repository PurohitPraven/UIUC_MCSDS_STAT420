...
colnames(msleep)
```
OR the below method can be used to see the observations and their meanings by using the printr package
```{r include=FALSE}
library(printr)
```
...
```{r}
mean(msleep[msleep$order=="Carnivora",]$bodywt)
```
...
colnames(birthwt)
```
OR the below method can be used to see the observations and their meanings
...
```
```{r include=TRUE}
The share of the histogram is slightly right skewed
```
