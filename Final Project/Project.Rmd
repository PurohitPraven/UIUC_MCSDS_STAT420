...
```{r}
m_small <- lm("log(SalePrice)~OverallCond", data = df_wo_influentials)

m_small_step <- step(m_small, scope = formula(m_no_influentials), direction = "forward", trace = 0)

```

We will now look at diagnostics data with these two models
...
df_result <- rbind(m_no_influentials=m_no_influentials_result, m_small_step=m_small_step_result)

knitr::kable(df_result)
```


Most of the diagnostics are the same

To decide we will look at the anova test
```{r}
anova(m_small_step, m_no_influentials)
```


With most of the diagnostics being the same and the pvalue of anova test being fairly large, and the number of predictors being slightly less for the smaller model, we will use m_small_step our final model.
