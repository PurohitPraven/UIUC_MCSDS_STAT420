...
Looking at the p-value we can say that the larger model has atleast some of the parameters that have a significant relationship with the predictor
...
We will not look at the individual significant of the parameters of this model to see if we can eliminate any predictors

```{r}
a <- coef(summary(m_transform_1_step))[,"Pr(>|t|)"] 
```

We will be conservative and use $\alpha = .01$. We now identify the individual columns that have p-value of greater than .1 and remove them from the dataset to create another model

```{r}
names(a[a>.01])
```

We will select all non-categorical variables that have pvalue>.01 and will will choose only the categorical predictors to remove that have most of the categories with p-value greater than 1
...
```{r}
remove_cols3 <- c("LotShape", "LotConfig", "LandSlope", "Neighborhood", "Condition1", "YearRemodAdd", "Exterior1st", "ExterCond", "Foundation", "BsmtExposure", "BsmtFinType", "Heating", "Functional", "PavedDrive", "EnclosedPorch", "SaleType", "SaleCondition")
library("NLP")

f <- formula(m_transform_1_step)
a <- paste(f, collapse = '+')
strsplit(as.String(f), fixed = TRUE, split = "+")[[1]]

predictor_list <- strsplit(as.String(formula(m_transform_1_step)), fixed = TRUE, split = "+")[[1]]
predictor_list <- predictor_list[!(predictor_list %in% remove_cols3)]

frm1 <- paste("log(SakePrice) ~ ", paste(predictor_list, collapse = ' + '))
