...
print("Structure of the epa data set after conversion is as below ")
str(epa_df)
...
g1 <- ggplot(data=epa_df, aes(x=horse, y= CO2, group=type, col=type)) + geom_point() + ggtitle("Scatter Plot of CO2 vs horsepower from epa2017 dataset")
g1
```
...
```{r}
paste("Average change in CO2 for a one foot-pound per second increase is horse is", coef(m_co2_horse)[2])
```
...
- Make a scatterplot of `CO2` versus `horse`. Use a different color point for each vehicle `type`.
```{r fig.height=7, fig.width=15, message=FALSE}
g2 <- ggplot(data=epa_df, aes(x=horse, y= CO2, group=type, col=type)) + geom_point() + ggtitle("Scatter Plot of CO2 vs horsepower from epa2017 dataset")
g2
```
...
coef(m_co2_horse_type_add_param)
```
...
```
The lines seem to fit the model well and much better than a line with a single intercept but we still see that for cars and trucks the lines dont seem to intersect the points perfectly


- Give an estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car`. 
```{r}
paste("Average change in CO2 for a one foot-pound per second increase is horse for type car is", coef(m_co2_horse_type_add_param)[4])
```
...
```{r fig.height=7, fig.width=15, message=FALSE}
g3 <- ggplot(data=epa_df, aes(x=horse, y= CO2, group=type, col=type)) + geom_point() + ggtitle("Scatter Plot of CO2 vs horsepower from epa2017 dataset")
g3
```
...
coef(m_co2_horse_type_int_param)
...
```{r}
paste("Average change in CO2 for a one foot-pound per second increase is horse for type car is", coef(m_co2_horse_type_int_param)[5])
```
...
```
Since the p-value is less thatn $\alpha$ we prefer the three way interaction model
...
```{r results="hold"}
print(paste("Estimate in change using predict = ", diff(predict(m_hsp_int_3, newdata = data.frame(Age=c(20,21), Education=c("high", "high"), Income=c("low","low"), Sex=c("male","male"))))))

print(paste("Estimate in change using model coefficients = ", coef(m_hsp_int_3)[3+1] + coef(m_hsp_int_3)[5+1] + coef(m_hsp_int_3)[7+1] + coef(m_hsp_int_3)[8+1]))
```
...
In order to calcualte the expression we will substract the additive model with the values above and Rate = 2 from the additive model again with the values above and Rate = 1. This will result in all expressions except the one which has Rate (x3) to cancel out. Seeing this in an equation we say 
