...
```
Note: It doesnt matter if we use type in newdata or skip it completely. Since type is not part of our model, it will not make a difference
...
```{r}
m_co2_horse_type_add <- lm(formula = CO2~horse+type, data = epa_df)
```
...
```{r fig.height=7, fig.width=15, message=FALSE}
g2 + stat_smooth(method = lm, se=FALSE, size=2)
```
The lines seem to fit the model well and much better than a line with a single intercept


- Give an estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car`. 
```{r}
paste("Average change in CO2 for a one foot-pound per second increase is horse for type car is", coef(m_co2_horse_type_add)[2])
```
...
```{r}
newdata <- data.frame(horse=c(148), type=c("Both"))
predict(m_co2_horse_type_add, newdata = newdata, interval = c("prediction"), level = .9)
```
