...
We set the seed to ensure reproducible results for the below sumulation studies
```{r}
birthday = 19730321
set.seed(birthday)
```
...
In this study we will look at two things

1) We will look at a  and see how the F-statistic, p-value and $R^2$ changes across the above two models

2) We will see how changing the sigma value from 1 to 5 to 10, changes the F-statistic, p-value and $R^2$ within the same model, and across the Significant and non significant model

We will observe the results of our simulation and try and conclude what effects the absence of beta parameters and changing noise has on the predictibility of the response variable
...
We create a function that will simulate the data 2000 times and return the f_stat, p-value and $R^2$ for each simulation as a dataframe
...
Now we Simulate Significant and Non significant models for $\sigma = 5$
...
Based on the Table 1.1 and  we see that the null Hypothesis gets rejected all the time for Significant modle when Sigma is 1. This implies that there is a strong relationship between the explamatory varaiables and predictor when the noise is minimum (sigma=1). We see that the rejection ratio becomes smaller, or said differently, the relationship starts becoming less strong as the variability of the predictor ($\sigma$) increases. 
...
We will discuss the finidings from the above graphs in the discussion section below
...
```{r pvalcom, fig.height=7, fig.width=15, fig.cap="Figure 1.2 - Comparison of p-value across Significant and Non-Significant model with Sigma in (1,5,10)"}
par(mfrow = c(2,3))
...
curve( dunif(x, min = min(x), max=max(x)), col = "darkorange", add = TRUE, lwd = 3)
...
$F = {\sum_{i=1}^{𝑛} (\hat{y}_{1i} - \bar{y})^2/p-1 \over \sum_{i=1}^{𝑛} (y_i - \hat{y}_{1i})^2/n-p}$

Here $\hat{y}_{1i}$ can be defined as the estimated y of the Full/larger model and $\bar{y}$ can be defined as y of the null model. Now the further apart the values of these 2 y values, the larger the F statistic will be and the smaller the p-value will be. We will reject the null Hypothesis for large values of F.



Figure \@ref(fig:fstatcomp) 
...
df_train_RMSE <- as.data.frame(rbind("model 1" = c(mean(m1_sig1_df$train_RMSE),mean(m1_sig2_df$train_RMSE),mean(m1_sig4_df$train_RMSE)),
                       "model 2" = c(mean(m2_sig1_df$train_RMSE),mean(m2_sig2_df$train_RMSE),mean(m2_sig4_df$train_RMSE)),
                       "model 3" = c(mean(m3_sig1_df$train_RMSE),mean(m3_sig2_df$train_RMSE),mean(m3_sig4_df$train_RMSE)),
                       "model 4" = c(mean(m4_sig1_df$train_RMSE),mean(m4_sig2_df$train_RMSE),mean(m4_sig4_df$train_RMSE)),
                       "model 5" = c(mean(m5_sig1_df$train_RMSE),mean(m5_sig2_df$train_RMSE),mean(m5_sig4_df$train_RMSE)),
                       "model 6" = c(mean(m6_sig1_df$train_RMSE),mean(m6_sig2_df$train_RMSE),mean(m6_sig4_df$train_RMSE)),
                       "model 7" = c(mean(m7_sig1_df$train_RMSE),mean(m7_sig2_df$train_RMSE),mean(m7_sig4_df$train_RMSE)),
                       "model 8" = c(mean(m8_sig1_df$train_RMSE),mean(m8_sig2_df$train_RMSE),mean(m8_sig4_df$train_RMSE)),
                       "model 9" = c(mean(m9_sig1_df$train_RMSE),mean(m9_sig2_df$train_RMSE),mean(m9_sig4_df$train_RMSE))))

colnames(df_train_RMSE) <- c("Sigma = 1", "Sigma = 2", "Sigma = 4")


df_test_RMSE <- as.data.frame(rbind("model 1" = c(mean(m1_sig1_df$test_RMSE),mean(m1_sig2_df$test_RMSE),mean(m1_sig4_df$test_RMSE)),
                      "model 2" = c(mean(m2_sig1_df$test_RMSE),mean(m2_sig2_df$test_RMSE),mean(m2_sig4_df$test_RMSE)),
                      "model 3" = c(mean(m3_sig1_df$test_RMSE),mean(m3_sig2_df$test_RMSE),mean(m3_sig4_df$test_RMSE)),
                      "model 4" = c(mean(m4_sig1_df$test_RMSE),mean(m4_sig2_df$test_RMSE),mean(m4_sig4_df$test_RMSE)),
                      "model 5" = c(mean(m5_sig1_df$test_RMSE),mean(m5_sig2_df$test_RMSE),mean(m5_sig4_df$test_RMSE)),
                      "model 6" = c(mean(m6_sig1_df$test_RMSE),mean(m6_sig2_df$test_RMSE),mean(m6_sig4_df$test_RMSE)),
                      "model 7" = c(mean(m7_sig1_df$test_RMSE),mean(m7_sig2_df$test_RMSE),mean(m7_sig4_df$test_RMSE)),
                      "model 8" = c(mean(m8_sig1_df$test_RMSE),mean(m8_sig2_df$test_RMSE),mean(m8_sig4_df$test_RMSE)),
                      "model 9" = c(mean(m9_sig1_df$test_RMSE),mean(m9_sig2_df$test_RMSE),mean(m9_sig4_df$test_RMSE))))


colnames(df_test_RMSE) <- c("Sigma = 1", "Sigma = 2", "Sigma = 4")
```



```{r fig.height=7}
ggplot(data = df_train_RMSE, aes(x=row.names(df_train_RMSE), y=df_train_RMSE$`Sigma = 1`, group = 1))+geom_point()+geom_line()+geom_line(data = df_test_RMSE, aes(x=row.names(df_test_RMSE), y=df_test_RMSE$`Sigma = 1`, group = 1, color=I("Orange")))+geom_point(data = df_test_RMSE, aes(x=row.names(df_test_RMSE), y=df_test_RMSE$`Sigma = 1`, group = 1, color=I("Orange")))
```
