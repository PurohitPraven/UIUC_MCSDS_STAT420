...
x_values <- seq(1, 10, length = n)
x1 <- x_predictors_df[,2]
x2 <- x_predictors_df[,3]
x3 <- x_predictors_df[,4]
...
sim_y_hats <- rep(0,num_simul)
sim_f_stats <- rep(0,num_simul)
sim_pvals <- rep(0,num_simul)
sim_r2s <- rep(0,num_simul)

for (i in 1:num_simul){
  epsilon <- rnorm(n,mean=mu, sd=sig)
  y_hat <- beta_0 + x1*beta_1 + x2*beta_2 + x3*beta_3 + epsilon
  m <- lm(y_hat~x1+x2+x3)
  sim_y_hats[i] <- y_hat
...
return(data.frame("y_hat"=sim_y_hats, "f_stat"=sim_f_stats, "pval"=sim_pvals, "r2"=sim_r2s))
