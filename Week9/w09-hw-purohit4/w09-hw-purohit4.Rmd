...
print(paste("predictors that are significant from model in 'b' are ", paste(pred_names, collapse = ", ")))
pred_formula <- paste(pred_names, collapse = "+")
full_formula <- paste("Employed~", pred_formula, sep = "")
m2 <- lm(full_formula, data = longley)
print("variance inflation factors for each of the predictors are as below")
(vif_m2 <- vif(m2))

print(paste("The predictor with the largest vif = ", names(vif_m2[max(vif_m2)])))
...
or that Employed can be expressed in terms of Unemplyed and Armed.Forces and Year
...
The distribution of the test statistic under null hypothesis will be an F-curve

todo - is this correct? or do we have to simulate data to show distribution?
...
todo - is this correct. Using the test results to conclude. Should we hide the function with echo=FALSE?
...
for(v in variables_to_try){
  frm2 <- paste((deparse(frm_1)), "+", v)
  m <- lm(frm2, data = Credit)
...
```{r}
(frm2 <- paste((deparse(frm_1)), "+", good_variables[1]))
mod_a <- lm(frm2, data = Credit)
```
...
Then use the step function and step back from the model to find the model with the lower AIC
...
print(paste("The RMSE for the selected model = ", get_loocv_rmse(mod_a)))
