...
```{r}
# Add BP_Test to above function and add better discription to result
...
Based on the above QQ plot above, we observe that most of the points fall on line. We do observe that it has fat tails, but the number of points on the tails are very less. Hence it is hard to say that the normality condition is met or violated by simply looking at the graph. To be sure we look at the BP_test results in the above table. Based on the BP_test we Fail to reject the Null Hypothesis that normality assumptions are not voilated. Hence we say that the Normality assumptions are met.
...
df1 <- rbind(coef(m1), coef(m2))
rownames(df1) <- c("original prostate dataset", "prostate dataset without infludential points")
knitr::kable(df1)

```

We see that there is slight change in all slopes, but nothing major. We also note that the sign of the intercept changes, which is a major change
...
df3 <- rbind(df1,df2)
row.names(df3) <- c("prediction of lpsa with influential points included", "prediction of lpsa with no influential points")
...
We see that the predicted values show minor change but no drastic changes are observed. This would imply that we would do nothing with the influential points and will keep them in the dataset 

todo - is the above assessment correct
...
Looking at the QQ plot and Residuals vs fit plot it is hard to make out if the linear model assumptions are met, but the results of the Shapiro and BP test results tell us that the assumptions are met hence we accept the Null Hypothesis that the Normality and equal variance assumptions are not violated for this model
...
Looking at the plots we see that plots for model 3 and model 4 seem to display constant variance. Since we prefer simplicity  we would prefer the third order model
...
Based on the result, since p value is larger than .1,  we still prefer the third model
...
```{r fig.width=15}
g1 <- ggplot(data = corrosion, aes(y=loss, x=Fe)) + geom_point() +
stat_smooth(method="lm", se=TRUE, fill=NA,
                formula=y ~ poly(x, 3, raw=TRUE),colour="red") + ggtitle(paste("Scatter plot beteeen loss and iron content with model ", summary(m3)$call)[2])
...
diagnostics2(m1, pcol = "black", testit = FALSE)
...
3) The Equal variance assumptions seems to be voilated when looking at the residuals vs fit plot
...
diagnostics2(m2, pcol = "black", testit = FALSE)

```

looking at the diagnostic charts, the QQ and residual vs fitted charts, we see that both Normality and equal variance assumptions are suspect. We do notice that the lower tail of the QQ plot has improved after converting price to log
...
diagnostics2(m3, pcol = "black", testit = FALSE)

```

The diagnostics of the model have improved a lot from the original dataset where the Normal assumption seems less suspect with both the tails coming in, and the constant variance plot seems a lot better.
...
exp(predict(m3, newdata = newdata, interval = c("predict"), level = .99))
```
